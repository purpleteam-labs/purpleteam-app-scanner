...
/*
            function listContexts(listContextsDone) {
               var contextList;
                zaproxy.context.contextList(function (err, resp) {
                  console.log("First time listing context list");
                  Object.keys(resp.contextList).forEach(function (key) {
                     console.log(key, resp.contextList[key]);
                  })
                  listContextsDone(state.error);
                });
            },
*/            
            function newContext(newContext1Done) {
                zaproxy.context.newContext("NodeGoat Context", zapApiKey, function (err, resp) {
                  console.log("Resp from newContext: " + resp);
...
/*          
            function listContexts(listContextsDone) {
               var contextList;
                zaproxy.context.contextList(function (err, resp) {
                  console.log("Second time listing context list");
                  Object.keys(resp.contextList).forEach(function (key) {
                     console.log(key, resp.contextList[key]);
                  })
                  listContextsDone(state.error);
...
/*            
            function listContexts(listContextsDone) {
               var contextList;
                zaproxy.context.contextList(function (err, resp) {
                  console.log("Third time listing context list");
                  Object.keys(resp.contextList).forEach(function (key) {
                     console.log(key, resp.contextList[key]);
                  })
                  listContextsDone(state.error);
...
*/            
            function includeInZapContext(includeInZapContextDone) {
                ...
                zaproxy.context.includeInContext("NodeGoat Context", zapTargetApp, zapApiKey, function(err, resp) {
 ...
 /*           
            function listContexts(listContextsDone) {
               var contextList;
                zaproxy.context.contextList(function (err, resp) {
                  console.log("Forth time listing context list");
                  Object.keys(resp.contextList).forEach(function (key) {
                     console.log(key, resp.contextList[key]);
                  })
                  listContextsDone(state.error);
