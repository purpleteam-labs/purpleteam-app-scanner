...
internals.applyReportStyling = ({ format, text }) => {
...
const numberOfAlertsForSesh = (alertCnt) => {
  ...
  else return internals.alertCount;
};

const createReports = async ({ testSessionId }) => {
  const { zaproxy, applyReportStyling, log, publisher } = internals;
  const { reportDir, reportFormats } = getProperties(['reportDir', 'reportFormats']);
  ...
  const zapApiReportFuncCallback = (response) => {
    ...
    toPrint.text = applyReportStyling(toPrint);

    return new Promise((resolve, reject) => {
      const reportFilePath = `${reportDir}report_appScannerId-${testSessionId}_${strings.NowAsFileName()}.${toPrint.format}`;
      log.info(`Writing ${toPrint.format}report to ${reportFilePath}, for Test Session with id: "${testSessionId}".`, { tags: ['zap'] });

      fsPromises.writeFile(reportFilePath, toPrint.text).then(() => {
        publisher.pubLog({ testSessionId, logLevel: 'info', textData: `Done writing ${toPrint.format}report file, for Test Session with id: "${testSessionId}".`, tagObj: { tags: ['zap'] } });
        resolve(`Done writing ${toPrint.format}report file.`);
      }).catch((writeFileErr) => {
        publisher.pubLog({ testSessionId, logLevel: 'error', textData: `Error writing ${toPrint.format}report file to disk, for Test Session with id: "${testSessionId}": ${writeFileErr}.`, tagObj: { tags: ['zap'] } });
        reject(new Error(`Error writing ${toPrint.format}report file to disk, for Test Session with id: "${testSessionId}": ${writeFileErr}`));
      });
    });
  };

  publisher.pubLog({ testSessionId, logLevel: 'info', textData: `About to write reports in the following formats, for Test Session with id: "${testSessionId}": ${[...reportFormats]}.`, tagObj: { tags: ['zap'] } });

  const reportPromises = reportFormats.map((format) => zaproxy.core[`${format}report`]().then(zapApiReportFuncCallback, (err) => `Error occurred while attempting to create Zap ${format} report, for Test Session with id: "${testSessionId}". Error was: ${err.message}.`));
